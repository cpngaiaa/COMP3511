...
enum SchedPolicy { 
  SCHED_FCFS,     // 5.3.1 First-come first-served scheduling (MP1)
    SCHED_SJF,    // 5.3.2 Shortest job first scheduling 
    SCHED_PRIO_NP,// 5.3.3 Nonpreemptive Priority scheduling (MP1)
    SCHED_PRIO_P, // 5.3.3 Preemptive     Priority scheduling (MP1)
    SCHED_RR,     // 5.3.4 Round robin scheduling (MP1)
    SCHED_MLQ,    // 5.3.5 Multilevel queue scheduling (MP1)
...
class Scheduler {
  ...
  private:
    ...
    Timer * ptimer;                // A timer, if needed.
};
