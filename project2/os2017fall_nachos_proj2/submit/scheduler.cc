    	...
    	case SCHED_SJF:
			{
				...
				int sortKey = thread->getBurstTime();
        			readyList->SortedInsert(thread,sortKey);
				break;
				
			}
    	case SCHED_PRIO_NP:
			{	//YOUR PROJECT2 CODE HERE
				int sortKey = 21-thread->getPriority();
        			readyList->SortedInsert(thread,sortKey);
				break;
			}
    	case SCHED_PRIO_P:
...
Scheduler::ShouldISwitch ( Thread  * oldThread, Thread * newThread )
{
   ...
   switch(policy)
   {
	...
	{	
		//YOUR PROJECT2 CODE HERE
		int oldPrior = oldThread->getPriority();
		int newPrior = newThread->getPriority();
