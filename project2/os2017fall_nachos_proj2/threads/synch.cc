...
Semaphore signal_counter("signal_counter", 1), signal_writer("signal_writer", 1);
int readercount = 0;
...
void Semaphore::P()
{
 ...
 //       printf("    %s hit semaphore %s but starts waiting.\n",currentThread->getName(), name);
        queue->Append(currentThread);	// so go to sleep
        currentThread->Sleep();
//        printf("    %s assumed from waiting for semaphore %s and get it.\n",currentThread->getName(), name);
    }
    else
    {
//        printf("    %s hit semaphore %s and get it successfully.\n",currentThread->getName(), name);
    }
